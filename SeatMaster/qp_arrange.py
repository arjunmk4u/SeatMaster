...
def generate_summaries(seating_df, ordered_rooms):
    ...
    # -------------------------
    # QP detailed counts (bench-seat)
    # -------------------------
    if not qp_summary_df.empty:
        qp_count_df = (
            qp_summary_df
            .groupby(["Room", "Subject"])
            .agg(
                QP_Needed=("Subject", "count"),
                Bench_Seat_Locations=(
                    "Bench",
                    lambda x: ", ".join(
                        f"{b}-{s}"
                        for b, s in zip(x, qp_summary_df.loc[x.index, "Seat"])
                    )
                )
            )
            .reset_index()
        ...
        qp_count_df = pd.DataFrame(
            columns=["Room", "Subject", "QP_Needed", "Bench_Seat_Locations"]
        )

    # -------------------------
    # Hall QP summary (existing)
    # -------------------------
    if not qp_summary_df.empty:
        hall_qp_summary_df = (
            qp_summary_df
        ...
        hall_qp_summary_df = pd.DataFrame(
            columns=["Room", "Subject", "Total QPs Needed"]
        )

    # -------------------------
    # ✅ NEW: Room × Subject × Students (YOUR SCREENSHOT)
    # -------------------------
    if not qp_summary_df.empty:
        room_subject_count_df = (
            qp_summary_df
            .groupby(["Room", "Subject"])
            .size()
        ...
        room_subject_count_df = pd.DataFrame(
            columns=["Room", "Subject", "Students"]
        )
    ...
    return (
        ...
        qp_count_df,
        hall_qp_summary_df,
        room_subject_count_df,   # 👈 NEW RETURN
    )
